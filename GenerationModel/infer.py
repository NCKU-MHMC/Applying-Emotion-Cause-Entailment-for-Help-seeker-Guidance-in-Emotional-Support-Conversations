...
parser.add_argument('--gold', action='store_true', help='use gold strategy id')
...
for infer_idx, infer_input_file in enumerate(args.infer_input_file):
    ...
    if not args.only_encode:
        if args.gold:
            save_dir = f'{checkpoint_dir_path}/res_{checkpoint_name}_{infer_input_file_name}_g_k.{args.top_k}' \
    ...
    import codecs
    with codecs.open(os.path.join(save_dir, f'gen.json'), 'w') as f:
    ...
    with open(os.path.join(save_dir, f'gen_only.txt'), 'w') as f:
        for line in res:
            f.write(line['generation'] + '\n')
