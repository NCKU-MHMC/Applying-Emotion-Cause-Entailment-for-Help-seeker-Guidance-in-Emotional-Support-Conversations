...
DISCRIMINATOR_MODELS_PARAMS = {
    ...
    "Strategy": {
        "path": "../StrategyClassifier_head/output_dialogpt/ESC_classifier_head_epoch_6.pt",
        "class_size": 8,
        "embed_size": 768,
        "pretrained_model": "../GenerationModel/DATA/strat_pp.strat_dialogpt/2024-04-22115423.5e-05.16.1gpu/epoch-3.bin",
        "class_vocab": {"Information": 0, "Others": 1, "Restatement or Paraphrasing": 2,
                        "Providing Suggestions": 3, "Affirmation and Reassurance": 4, "Self-disclosure": 5,
                        "Question": 6, "Reflection of feelings": 7}
    },
    "Emotion_GO": {
        "path": "../EmotionClassifier_head/output_dialogpt/GO_classifier_head_epoch_8.pt",
        "class_size": 28,
        "embed_size": 768,
        "pretrained_model": "../GenerationModel/DATA/strat_pp.strat_dialogpt/2024-04-22115423.5e-05.16.1gpu/epoch-3.bin",
        "class_vocab": {"relief": 0, "disgust": 1, "curiosity": 2, "remorse": 3, "nervousness": 4,
                        "desire": 5, "embarrassment": 6, "caring": 7, "excitement": 8, "annoyance": 9,
                        "disappointment": 10, "sadness": 11, "disapproval": 12, "pride": 13, "anger": 14,
                        "love": 15, "optimism": 16, "neutral": 17, "realization": 18, "approval": 19,
                        "confusion": 20, "fear": 21, "joy": 22, "amusement": 23, "admiration": 24,
                        "gratitude": 25, "grief": 26, "surprise": 27}
    }
    ...
    for i in range(num_iterations):
        ...
        if loss_type == TYPE_ENGAGEMENT: # TODO, not included for ALL fow now.
            ...
            for _ in range(horizon_length):
                ...
                predict_accumulated_hidden = new_accumulated_hidden + torch.sum(
                    curr_hidden, dim=1)
        ...
        if loss_type == TYPE_strategy or loss_type == PPLM_ALL and target_strategy and str_w != 0:      
            ...
            if verbosity_level >= VERY_VERBOSE: 
                ...
                strategy_loss_record += strategy_loss.data.cpu().numpy()  
                loss_list.append(strategy_loss)

        ### ===strategy attribute model end=== 
        
        # calculating Emotion attribute loss
        emo_loss = 0
        if loss_type == TYPE_EMOTION or loss_type == PPLM_ALL and emo_w != 0:
            ...
            _, _, context_all_hidden = model(context_t,
                                            output_hidden_states=True,
                                            return_dict=False) 
            ...
            if verbosity_level >= VERY_VERBOSE:
                ...
                emo_loss_record += emo_loss.data.cpu().numpy()          
        ...
        if verbosity_level >= VERY_VERBOSE:
            ...
            kl_loss_record += kl_loss.data.cpu().numpy()
...
def generate_text_pplm(
    ...
    for i in range_func:
        ...
        if not perturb or num_iterations == 0:
            ...
            context_range = output_so_far.size(1) - context_t.size(1)
            accumulated_hidden = unpert_last_hidden[:, context_range:-1, :]
...
def run_pplm_example(
    ...
    global file_loss
    if rsa and page:
        out_name = '/DPPRSA_dialogpt_' + attribute_type + '_x' + str(num_iterations) + '_ew' + str(emo_weight) + '_sw' + str(str_weight)
    elif rsa:
        out_name = '/PPRSA_dialogpt_' + attribute_type + '_x' + str(num_iterations) + '_ew' + str(emo_weight) + '_sw' + str(str_weight)
    elif page:
        out_name = '/DPPLM_dialogpt_' + attribute_type + '_x' + str(num_iterations) + '_ew' + str(emo_weight) + '_sw' + str(str_weight)
    elif attribute_type == 'all':
        out_name = '/PPLM_dialogpt_' + attribute_type + '_x' + str(num_iterations) + '_ew' + str(emo_weight) + '_sw' + str(str_weight) 
    elif attribute_type == 'emotion':
        out_name = '/PPLM_dialogpt_' + attribute_type + '_x' + str(num_iterations) + '_ew' + str(emo_weight)
    elif attribute_type == 'strategy':
        out_name = '/PPLM_dialogpt_' + attribute_type + '_x' + str(num_iterations) + '_sw' + str(str_weight)
    else:
        out_name = '/DialoGPT-Joint-pplm'
        num_iterations = 0
    ...
    if for_test_run:
        ...
        file_loss = open('output/' + out_name + '_loss.txt', 'w+', encoding='utf-8')
    ...
    if page:
        file_loss.write('neutral:' + str(neutral_count)+' \n')
        file_loss.write('self-contagion:' + str(self_count)+' \n')
        file_loss.write('inter-personal:' + str(inter_count)+' \n')
    
    file_loss.write('inter-total time(second):' + str(total_time) +' \n')
    file_loss.write('strategy prediction accurracy:' + str(round(strat_pred_acc/sentence_count,2)) +' \n')
