...
class Discriminator(torch.nn.Module):
    ...
    def avg_representation(self, x, attention_mask):
        if hasattr(self.encoder, 'transformer'):                   
            ...
            decode_x = torch.cat((decoder_input_ids,x), 1)
            ...
            mask = decode_x.ne(0).unsqueeze(2).repeat(
                1, 1, self.embed_size
            ).float().to(self.device).detach()
...
def train_epoch(data_loader, discriminator, optimizer,
    ...
    pos_weight = torch.ones([28]).to(device)
...
def train_discriminator(
    ...
    idx2class = {'admiration', 'amusement', 'anger', 'annoyance',  'approval',
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="GO")
